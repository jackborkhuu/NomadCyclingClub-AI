...
function setupYammerFeed() {
  ...
  if (!LOUNGE_CONFIG.yammerGroupId && !LOUNGE_CONFIG.yammerGroupToken) {
    statusNode.textContent = 'Set yammerGroupId or yammerGroupToken in club-lounge.js to enable the Discussions feed.';
    return;
  }

  statusNode.textContent = 'Loading Viva Engage discussion feed...';

  if (window.yam && window.yam.connect && window.yam.connect.embedFeed && LOUNGE_CONFIG.yammerGroupId) {
  ...
  if (LOUNGE_CONFIG.yammerGroupToken) {
    ...
    yammerTarget.innerHTML = `<p class="yammer-fallback-link"><a href="${feedUrl}" target="_blank" rel="noopener">Open Nomad Discussions feed</a></p>`;
    statusNode.textContent = 'Direct feed link loaded. Open discussions in a new tab.';
    return;
  }

  statusNode.textContent = 'Viva Engage embed script did not load. Add yammerGroupToken for direct feed link fallback.';
}
