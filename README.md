...
This project now includes a race-director-managed race system with:
...
- Publish and close actions from the race management UI

### Files

- `club-lounge.html` - members lounge (discussions + calendar)
- `race-management.html` - dedicated RaceDirectors race management UI
...
- `RACE_DIRECTORS_GROUP_OBJECT_ID` - Microsoft 365 group object ID for race director access
- `BOARD_GROUP_OBJECT_ID` - legacy fallback name for the same group setting (backward compatibility)
...
- `/api/race-admin/config` checks if signed-in user is in the RaceDirectors group.
- Race directors can create tournaments, stages, riders, results, publish, and close.
- Google sync is optional and only imports riders that are not already present.
...
- The API auto-creates the list on first race director access via bootstrap.
