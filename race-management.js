...
async function initRaceManagement() {
  ...
  try {
    ...
    if (!config.canManageBoard) {
      app.hidden = true;
      setRaceStatus('Race management is available to Microsoft 365 Board group members only.', true);
      return;
    }

    setRaceStatus('Board access granted. Loading race management...');
    app.hidden = false;
...
document.addEventListener('DOMContentLoaded', () => {
  if (!window.location.pathname.toLowerCase().endsWith('club-lounge.html')) {
    return;
  }
  initRaceManagement();
});
